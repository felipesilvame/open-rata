...
class ScrapAbcdin implements ShouldQueue
{
    ...
    public function __construct(array $categories, string $discount, int $delay, string $webhook)
    {
        ...
        $this->protocol = 'https';
        $this->method = 'GET';
        $this->uri = 'www.abcdin.cl/on/demandware.store/Sites-Abcdin-Site/es_CL/Search-UpdateGrid?cgid=';
    ...
    {
        Log::info("ScrapAbcdin: Iniciando Job");
        ...
        foreach ($this->categories as $category) {
            ...
            try {
                ...
                if ($client->getResponse()->getStatusCode() !== 200) {
                    Log::error("ScrapAbcdin: Error al obtener la página de la categoría $category");
                    continue;
                }
            } catch (\Throwable $th) {
                Log::error("ScrapAbcdin: Error al obtener la página de la categoría $category");
            ...
            try {
                ...
                Log::error("ScrapAbcdin: Error al obtener los productos de la categoría $category");
                throw $th;
            }
            try {
                $data = collect($items->each(function($node){
                    ...
                    try {
                        ...
                        $url = 'https://www.abcdin.cl'.$node->filter($this->buy_url_field)->first()->attr('href');
                    ...
                    if ($nombre && $sku && $p_normal && $img) {
                        ...
                        //Log::warning("ScrapAbcdin: No se pudo obtener la información de un producto. Nombre: $nombre, SKU: $sku, Precio normal: $p_normal, Url: $url");
                        return null;
                    }
                }));
                Log::info("ScrapAbcdin: Se obtuvieron ".count($data)." productos de $this->tienda para la categoria $category");
                ...
                foreach($filtered->values() as $item) {
                    ...
                    if (!$sospecha) {
                        ...
                        try {
                            ...
                            Log::error("ScrapAbcdin: Error al enviar la sospecha rata por discord. \n"+ $th->getMessage() + " \n"+ $th->getTraceAsString());
                            //throw $th;
                        }
                    }
                }
            } catch (\Throwable $th) {
                Log::error("ScrapAbcdin: Error al procesar los productos de la categoría $category. \n"+ $th->getMessage() + " \n"+ $th->getTraceAsString());
                //throw $th;
            }
        }
    }
